...
const (
	cwEnclaveAppURL = "http://localhost:8088"
	cwTestNonce     = "abc1111111111111111111111111111111111111"
	cwTestUserPIN   = "my-secure-pin-123456"
)

type CWCreateWalletResponse struct {
...
func fetchRawPubKeyCW() (string, error) {
	fmt.Printf("[Step 0] 正在从业务接口获取公钥原文...\n")
	resp, err := http.Get(cwEnclaveAppURL + "/tee_wallet/tee_pubkey")
	if err != nil {
...
func fetchCWRootPubKeyFromAttestation(nonce string, rawPubKeyHex string) (string, error) {
	...
	// 请求业务接口的证明
	url := fmt.Sprintf("%s/tee_wallet/attestation?nonce=%s", cwEnclaveAppURL, nonce)
...
func encryptPasswordCW(rootPubKeyHex string, password string) (string, error) {
...
// 4. 验证响应签名逻辑
func verifySignatureCW(pubKeyHex, nonce, signedNonceBase64 string) error {
...
// 5. PublicKeyToEthAddressCW 将压缩公钥转换为以太坊地址
func PublicKeyToEthAddressCW(pubKeyHex string) (string, error) {
...
func main() {
	...
	// A. 先从业务端口拿公钥原文
	rawPubKey, err := fetchRawPubKeyCW()
	if err != nil {
		log.Fatalf("❌ 步骤 0 失败: %v", err)
	}

	// B. 拿证明文档比对哈希，确保证明该公钥来自合法 Enclave
	verifiedPubKey, err := fetchCWRootPubKeyFromAttestation(cwTestNonce, rawPubKey)
	if err != nil {
		log.Fatalf("❌ 步骤 1 失败: %v", err)
	}

	// C. 使用验证过的公钥加密用户 PIN
	encryptedPass, err := encryptPasswordCW(verifiedPubKey, cwTestUserPIN)
	if err != nil {
	...
	resp, err := http.Post(cwEnclaveAppURL+"/tee_wallet/create_key_share", "application/json", bytes.NewBuffer(jsonData))
	if err != nil {
	...
	var walletResp CWCreateWalletResponse
	...
	fmt.Printf("\n[Step 3] 收到响应，验证签名...\n")
	err = verifySignatureCW(verifiedPubKey, cwTestNonce, walletResp.SignedNonce)
	if err != nil {
		fmt.Printf("❌ 验签失败: %v\n", err)
	} else {
		...
		// 生成并显示 ETH 地址
		ethAddr, err := PublicKeyToEthAddressCW(walletResp.WalletPublicKey)
		if err != nil {
